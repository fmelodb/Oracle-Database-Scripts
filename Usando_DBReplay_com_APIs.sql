...
ToDo: incluir passos quando ambiente for RAC
...
SELECT max(id) from DBA_WORKLOAD_CAPTURES;

BEGIN
  DBMS_WORKLOAD_CAPTURE.EXPORT_AWR (capture_id => <id>);
END;
/
...
CREATE USER capture_awr IDENTIFIED BY ... 
...
SELECT DBMS_WORKLOAD_CAPTURE.IMPORT_AWR (capture_id => <id>,  -- utilize o mesmo ID do export do AWR
...
BEGIN
  DBMS_WORKLOAD_REPLAY.EXPORT_AWR(replay_id => <replay id>);
END;
/
...
VAR v_clob CLOB
BEGIN
	dbms_workload_replay.compare_period_report(replay_id1 => <replay id>,
...
BEGIN
   l_result := dbms_workload_replay.compare_sqlset_report(replay_id1 => <replay id>,
                                                          replay_id2 => NULL, -- compara contra a captura
                                                          format => DBMS_WORKLOAD_REPLAY.TYPE_HTML,
                                                          result => :v_clob);
END;
