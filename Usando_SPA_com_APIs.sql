...
BEGIN
  OPEN cur FOR
  SELECT VALUE(P) FROM table(DBMS_SQLTUNE.SELECT_WORKLOAD_REPOSITORY(<snap_id>,<snap_id>)) P; -- trocar snaps ini e fim
...
EXEC DBMS_SQLTUNE.REMAP_STGTAB_SQLSET ( staging_table_name => 'MY_STAGING_TABLE', -
					...
					old_con_dbid => <source condbid>, -
					new_con_dbid => <target condbid>);
